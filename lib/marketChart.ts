...
export function initMarketChart(
  ...
  const cfg = data.marketAnalysis || {};
  const volumes = data.performance3d || [];
  ...
  function chartArea(w: number, h: number) {
  ...
  function yScale(val: number, min: number, max: number, lineTop: number, lineBottom: number) {
    const t = (val - min) / (max - min || 1);
    return lineBottom - t * (lineBottom - lineTop);
  }

  function xAt(i: number, n: number, left: number, cw: number) {
    if (n <= 1) return left + cw / 2;
    return left + (i / (n - 1)) * cw;
  }

  function draw(progressT: number) {
    const dpr = Math.min(window.devicePixelRatio, 2);
    const rect = canvasEl.getBoundingClientRect();
    const w = rect.width || panelEl.clientWidth;
